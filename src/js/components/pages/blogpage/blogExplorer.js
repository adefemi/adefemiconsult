...
const initialState = {
    ...
    commentRemoveSubmit: false, pageLoading: true, reactionData: null, reactionType: "post", reactionID: null, activeIP: null
};
...
class BlogExplorer extends React.Component{
    ...
    verifyViewers(){
        if(this.state.blogView === null || this.state.activeIP === null || this.state.activeBlog === null){
            setTimeout(() => {
                this.verifyViewers();
            }, 5000);
            return;
        }
        let blogViews = [...this.state.blogView];
        blogViews = blogViews.filter(o => o.ip === this.state.activeIP);
        if(blogViews.length > 0) return;
        let payload = new FormData();
        payload.append("blog_id", this.state.activeBlog[0].id);
        payload.append("ip", this.state.activeIP);

        this.props.actionWithData('post', this.props.backEndLinks.blogView, payload).then(
            res => {
    ...
    getBlogs(){
        ...
        for(let i = 0; i < _blogContent.length; i++){
            _tempContent.push(
                <div key={i} className={'video-card'} onClick={() => this.props.history.push('/blog/'+_blogContent[i].slug)}>
            ...
            <div className={'wrapper'}>
                ...
                <div className={'wrapper'}>
                    <div className={'blog-container'}>
                        <SideBar tagLink={'/blog'} tagList={this.state.activeBlog === null || this.state.activeBlog.length < 1? null :
                            ...
                            <div className={'content-display'}>
                                {
                                    this.state.activeBlog === null || this.state.pageLoading ? <div className={'left'}><SubLoader/></div> :
                                        this.state.activeBlog.length < 1 ?
                                            <div className={'left'}><h4 className={'main-notify'}>The blog content you're looking for has either been removed or doesn't exist!</h4></div>:
                                            <div className={'video-explorer'}>
                                                ...
                                                <div className={'left'}>
                                                    ...
                                                    <a href={this.state.activeBlog[0].coverpic}>
                                                        <div className={'image-con'}>
                                                            <ImageLoader/>
                                                ...
                                                <div className={'right'}>
                                                    ...
                                                    </div>
                                                    {
                                                        this.state.activeBlog === null || this.state.activeBlog.length < 1 ? <SubLoader/> :
                                                            this.getBlogs().length < 1 ? <h4 className={'main-notify'}>No blog content found!</h4>:
                                                                this.getBlogs()
                                                    }
                                                </div>
                                            </div>
