            ...
            <div className={'wrapper'}>
                ...
                <div className={'wrapper'}>
                    <div className={'blog-container'}>
                        <SideBar activeSelection={this.state.querystring} tagLink={'/blog/video'} tagList={this.state.blogTag === null ? null : this.state.blogTag} type={'video'}/>                        <div className={'mainContent'}>
                            <div className={'content-display'}>
                                <div className={'head'}>Video Tutorials
                                    {
                                        this.state.querystring === null ? null :
                                            this.state.queryType === "genre" ?
                                                <span>[Showing video post of type <strong>{this.state.querystring}</strong>]</span>
                                                :
                                                <span>[Showing video post with tag <strong>{this.state.querystring}</strong>]</span>
                                    }
                                </div>
                                <div className={'video-container'}>
                                    {
                                        this.state.blogs === null || this.state.pageLoading ? <SubLoader/> :
                                            this.getBlogs().length < 1 ? this.state.querystring !== null ? <h4 className={'main-notify'}>No video content was found under [{this.state.querystring}]</h4> :
                                                <h4 className={'main-notify'}>No Video post was found!</h4> :
                                                this.getBlogs()
                                    }
